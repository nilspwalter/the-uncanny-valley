...
def get_embedding_matrix(model):
    ...
    elif "FalconForCausalLM" in str(type(model)):
...
class AttackPrompt(object):
    ...
    def test_loss(self, model):
        logits, hidden_state, ids = self.logits(model, return_ids=True)
    ...
    def logits(self, model, test_controls=None, return_ids=False):
        ...
        if return_ids:
            ...
            output = model(input_ids=ids, attention_mask=attn_mask)
            return output.logits, output.hidden_states, ids
        else:
            del locs, test_ids
...
class MultiPromptAttack(object):
    ...
    def get_filtered_cands(self, worker_index, control_cand, filter_cand=True, curr_control=None):
        ...
        for i in range(control_cand.shape[0]):
            ...
            if filter_cand:
                ...
                #print(worker.tokenizer(decoded_str, add_special_tokens=True))
                #print(worker.tokenizer)
                #print(len(worker.tokenizer(decoded_str, add_special_tokens=True).input_ids),len(control_cand[i]))
                if "PreTrainedTokenizerFast" in str(type(worker.tokenizer)):
                    decoded_2 = worker.tokenizer.decode(worker.tokenizer(decoded_str, add_special_tokens=False).input_ids, skip_special_tokens=True)
                    if decoded_str != curr_control:
                        cands.append(decoded_str)
                    else:
                        count += 1
                else:
                    if decoded_str != curr_control and len(worker.tokenizer(decoded_str, add_special_tokens=True).input_ids) == len(control_cand[i]):
                        cands.append(decoded_str)
                    else:
                        count += 1
            else:
                cands.append(decoded_str)
        #print(control_cand,decoded_str) 
        if filter_cand:
    ...
    def run(self, 
        ...
        for i in range(n_steps):
            ...
            if False:
                model_tests_jb, model_tests_mb, model_tests_loss = self.test(self.workers, self.prompts, include_loss=True)
                if all(all(tests for tests in model_test) for model_test in model_tests_jb):
                    print(model_tests_jb)
                    self.log(i+1+anneal_from, n_steps+anneal_from, self.control_str, best_loss, runtime, (model_tests_jb, model_tests_mb, model_tests_loss), verbose=verbose)
                    

            steps += 1
            start = time.time()
            torch.cuda.empty_cache()
            control, fam, loss = self.step( # returns fam
                ...
                verbose=verbose, return_fam=return_fam
            )
            fams.append(fam)
            losses.append(loss)
        ...
        if return_fam:
            return self.control_str, losses, fams, steps
        return self.control_str, loss, steps # return fam here
...
class IndividualPromptAttack(object):
    ...
    def run(self, 
        ...
        for i in range(len(self.goals)):
            ...
            control_str, losses, fams, steps = attack.run(
                n_steps=n_steps,
                batch_size=batch_size,
                topk=topk,
                temp=temp,
                ...
                log_first=True,
                filter_cand=filter_cand,
                verbose=verbose, return_fam=True
            )
            ...
            ax[1].plot(losses)
            ax[1].set_title("Loss")
            fig.savefig("../results/plots/{}-{}.png".format(name,self.offset+i))
            np.save("../results/fams-loss/fam-{}-{}.npy".format(name,self.offset+i),fams)
            np.save("../results/fams-loss/loss-{}-{}.npy".format(name,self.offset+i),losses)
