...
def compute_fam(logits,phis):
    # output bs x tokens or just bs x 1
    bs,ntokens,_ = logits.shape

    fams_per_sample = torch.zeros(size=(bs,1))
    for b in range(bs):
        temp = torch.zeros(size=(ntokens,1))
        for nt in range(ntokens):
            logs = (logits[b][nt]).detach()
            phi = phis[b][nt].detach()
            logs = torch.softmax(logs.unsqueeze(1),dim=0)
            rightterm = (phi*phi).sum()
            leftterm = torch.sum(logs*(1-logs))
            temp[nt] = leftterm * rightterm 
        fams_per_sample[b] = temp.mean()
    return fams_per_sample
...
class GCGMultiPromptAttack(MultiPromptAttack):
    ...
    def step(self, 
             ...
             verbose=False, 
             opt_only=False,
             filter_cand=True, return_fam=False):
        ...
        with torch.no_grad():
            for j, cand in enumerate(control_cands):
                ...
                for i in progress:
                    ...
                    logits, hidden_states, ids = zip(*[worker.results.get() for worker in self.workers])
                    ...
                    fam = compute_fam(logits[0],hidden_states[0]).detach()
        ...
        #print("Flatness measure", float(fam))
        #print('Current length:', len(self.workers[0].tokenizer(next_control).input_ids[1:]))
        #print(next_control)
        #TODO compute flatness here
        if return_fam:
            return next_control, float(fam), cand_loss.item() / len(self.prompts[0]) / len(self.workers)
