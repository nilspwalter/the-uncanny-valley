...
def adv_pgd_trade_off(
    ...
    clip_max,
    distance="l_inf",
    natural_criterion=nn.CrossEntropyLoss(), alpha=-1
):
    model.eval()
    optimizer.zero_grad()
    x_adv = pgd_whitebox(model, x_natural,y,device,epsilon, perturb_steps,step_size,clip_min, clip_max)
    optimizer.zero_grad()
    model.train()
    logits_nat = model(x_natural)
    logits_adv = model(x_adv)

    loss_nat = natural_criterion(logits_nat, y)
    loss_adv = natural_criterion(logits_adv, y)
    return alpha * loss_nat + (1-alpha) * loss_adv
