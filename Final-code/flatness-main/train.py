...
def main():
    ...
    if os.path.exists(result_main_dir):
        ...
        print(os.path.exists(result_main_dir))
        print(result_main_dir)
        ...
        print("hi")
        while os.path.exists(result_sub_dir):
            print(result_sub_dir)
            shutil.rmtree(result_sub_dir, ignore_errors=True)
            print(os.path.exists(result_sub_dir))
            time.sleep(1)
    else:
    ...
    for epoch in range(args.start_epoch, args.epochs + args.warmup_epochs):
        ...
        if args.val_method == "mixtrain" and epoch <= args.schedule_length:
            ...
            a= val(model, device, test_loader, criterion, args, writer, epoch=epoch)
            print(len(a))
            prec1 = a[0]
        else:
