...
class RelativeFlatness():
    ...
    def __call__(self, inputs, targets, approximate):
        ...
        if self.norm_function == 'neuronwise':
            ...
            hessian = self.hessian_function.trace(inputs, targets, approximate, device)
            regularization = weights_norm * hessian
            return weights_norm, hessian
        ...
        elif self.norm_function == 'diagonal':
            H = self.hessian_function(inputs, targets, device).reshape(np.prod(shape), np.prod(shape))
            weights_norm = torch.linalg.norm(self.feature_layer,dim=1)
            diagonal = torch.diag(H)
        return weights_norm,diagonal
...
class LayerHessian():
    def __init__(self, model, layer_id, loss_function, method='functorch'):
        ...
        for i, p in enumerate(self.model.named_parameters()):
            #print(i, p[0])
            if p[0] == "fc.weight" or p[0] == "linear.weight":#"block3.layer.3.conv1.weight":
                self.f_name = p[0]
                self.f_layer = p[1]
                self.layer_id = i
                #print(i)
    ...
    def __call__(self, inputs, targets, device):
        if self.method == 'functorch':
            ...
            #print("Loss", loss)
            #print(torch.sum(grad(loss, self.f_layer, create_graph=True, retain_graph=True)[0]))
            #print(torch.trace(hessian.reshape(np.prod(shape), np.prod(shape))))
            #print()
            return hessian
        else:
        ...
        if approximate:
            ...
            for i in range(500):
                ...
                if abs(torch.mean(torch.tensor(trace_vhv)) - trace) / (abs(trace) + 1e-6) < tol:
                    ...
                    #print("Loss", loss)
                        #print(torch.sum(grad(loss, self.f_layer, create_graph=True, retain_graph=True)[0]))
                        #print(torch.trace(torch.mean(torch.tensor(trace_vhv, requires_grad=True)).reshape(np.prod(shape), np.prod(shape) )))
                    #print(torch.mean(torch.tensor(trace_vhv, requires_grad=True)))
                    #print()
            ...
            hessian = self(inputs, targets, device).reshape(np.prod(shape), np.prod(shape))
            #print(hessian.max(), hessian.min(), hessian.mean())
            return torch.trace(hessian)
