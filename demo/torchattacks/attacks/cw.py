    ...
    def forward(self, images, labels):
        ...
        for step in range(self.steps):
            ...
            if self.targeted:
                ...
                # If the attack is not targeted we simply make these two values unequal
                condition = (pre != labels).float()
            #print(step, torch.mean(condition.cpu()))
            # Filter out images that get either correct predictions or non-decreasing loss,
            # i.e., only images that are both misclassified and loss-decreasing are left
            mask = condition * (best_L2 > current_L2.detach())
            best_L2 = mask * current_L2.detach() + (1 - mask) * best_L2
            ...
            #        return best_adv_images,None
            #    prev_cost = cost.item()
            adv_images_trace.append(best_adv_images.detach().cpu())
        return best_adv_images, torch.stack(adv_images_trace)
...
class CW_NO(Attack):
    ...
    def forward(self, images, labels):
        ...
        for step in range(self.steps):
            ...
            #        return best_adv_images,None
            #    prev_cost = cost.item()
            adv_images_trace.append(adv_images.detach().cpu())
        return adv_images, torch.stack(adv_images_trace)
