...
from utils.adv import trades_loss, adv_pgd_trade_off

# TODO: add adversarial accuracy.
def train(
    model, device, train_loader, sm_loader, criterion, optimizer, epoch, args, writer
):
    if args.adv_trade_off:
            print(
        " ->->->->->->->->->-> One epoch with Adversarial training with trade-off {} <-<-<-<-<-<-<-<-<-<-".format(args.alpha)
    )
    else:
        print(
    ...
    for i, data in enumerate(dataloader):
        ...
        if args.adv_trade_off:
            ...
            distance="l_inf",
            natural_criterion=nn.CrossEntropyLoss(), alpha=args.alpha
        )
        else:
            print("Trades")
            loss = trades_loss(
